...
const KIND = {
  gatil: { label: 'Gatil', icon: '🐈' },
  canil: { label: 'Canil', icon: '🐕' },
  baia: { label: 'Baia', icon: '🐴' },
  quarentena: { label: 'Quarentena', icon: '⚠️' },
  outro: { label: 'Ambiente', icon: '🏠' }
};
...
const helpers = {
  ...
  kindIcon: (k) => (KIND[k] || KIND.outro).icon,
  speciesIcon: (s) => SPECIES_ICON[String(s || '').toLowerCase()] || '🐾',
